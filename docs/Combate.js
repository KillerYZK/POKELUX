...
function crearPokemonPorDefecto(nombre) {
  return {
    ...
    nivel:         50,
    movimientos:   Array(4).fill(null).map(() => crearMovimientoPorDefecto()),
    tipos:         ["normal"],
    shiny:         false,
    objeto:        null,
    objetoUsado:   false
...
async function cargarEquipo(listaEquipo, nombreJugador) {
  ...
  return Promise.all(listaEquipo.map(async (entry) => {
    const nombreBase = String(entry.nombre || (typeof entry === "string" ? entry : entry.id) || "").trim();
    if (!nombreBase || nombreBase === "undefined") return crearPokemonPorDefecto("Unknown");

    // ── Obtener configuración guardada ──
    let cfg = {};
    if (nombreJugador === miNombre) {
      cfg = JSON.parse(localStorage.getItem(`configuracion-${nombreBase}`)) || {};
    } else {
      try {
        const snap = await get(ref(db, `partidas/${partidaId}/configuraciones/${nombreJugador}/${nombreBase}`));
        cfg = snap.val() || {};
      } catch { /* silencioso */ }
    }
    ...
    try {
      ...
      const res     = await fetch(`https://pokeapi.co/api/v2/pokemon/${nombreBase.toLowerCase()}`, { signal: ctrl.signal });
      ...
      if (movNombres.length > 0) {
        ...
        movimientos = await Promise.all(movNombres.map(async (nombre) => {
          try {
            const r = await fetch(`https://pokeapi.co/api/v2/move/${nombre.toLowerCase()}`);
            ...
            return {
              ...
              efecto:       d.effect_entries?.find(e => e.language.name === "en")?.effect || "",
        ...
        movimientos = await Promise.all(movUrls.map(async (url) => {
          try {
      ...
      const pokemonData = {
        ...
        shiny:          esShiny,
        objeto:         objetoGuardado ? { id: objetoGuardado, nombre: objetoGuardado } : null,
        objetoUsado:    false
      };
  ...
  await set(ref(db, `partidas/${partidaId}/combate`), {
    ...
    pokemonData: {
      [miNombre]:          miEquipo.map(p    => ({
        nombre: p.nombre, nombreOriginal: p.nombreOriginal,
        hpMax: p.hpMax, stats: p.stats, tipos: p.tipos, nivel: p.nivel
      })),
...
async function iniciarCombate() {
  ...
  try {
    ...
    if (!miEquipo.length || !equipoRival.length) throw new Error("No se pudieron cargar los equipos");
...
function renderEstado(estado) {
  ...
  const miEstadoPoke    = estado.estados[miNombre]?.[miIndexActivo]          || { nombre: null };
  const rivalEstadoPoke = estado.estados[rivalNombreGlobal]?.[rivalIndexActivo] || { nombre: null };
  ...
  miEquipo.forEach((p, i) => { p.objetoUsado = objetosUsados.includes(i); });

  actualizarPokeballs("jugador", estado.hp[miNombre],          miEquipo,    estado.estados[miNombre],          miIndexActivo);
  actualizarPokeballs("enemigo", estado.hp[rivalNombreGlobal], equipoRival, estado.estados[rivalNombreGlobal], rivalIndexActivo);
...
function actualizarSprite(id, pokemon, lado) {
  ...
  if (!img) return;
  const nombreSprite = (pokemon.nombreOriginal || pokemon.nombre).toLowerCase().replace(/\s/g, "-");
  img.src = `${SPRITE_BASE[lado]}${nombreSprite}.gif`;
  img.alt = pokemon.nombre;
  img.onerror = () => { img.src = `${SPRITE_BASE[lado]}placeholder.png`; };
}
...
function actualizarInfobar(lado, pokemon, hpActual, estadoPoke) {
  ...
  if (nomEl) nomEl.textContent = pokemon.nombre.toUpperCase() + (estadoPoke?.nombre ? ` [${estadoPoke.nombre}]` : "");
...
function actualizarPokeballs(prefijo, hpArray, equipo, estadosArray, indexActivo) {
  equipo.forEach((_, i) => {
...
async function resolverAtaque(estado) {
  ...
  const pokeAtacante = pokDataAtac || (atacante === miNombre ? miEquipo[indexAtacante] : equipoRival[indexAtacante]);
  const pokeDefensor = pokDataDef  || (defensor  === miNombre ? miEquipo[indexDefensor] : equipoRival[indexDefensor]);
  ...
  // ── Animación visual ──
  const spriteAtacanteEl = document.getElementById(atacante === miNombre ? "jugador-sprite" : "enemigo-sprite")?.parentElement;
  const spriteDefensorEl = document.getElementById(atacante === miNombre ? "enemigo-sprite" : "jugador-sprite")?.parentElement;
  ...
  try {
    ...
    const resultado = await runTransaction(combateRef, (est) => {
      ...
      const movFB      = est.movimientos?.[atacante]?.[indexAtacante]?.[accion.movIndex];
      const pokAtacFB  = est.pokemonData?.[atacante]?.[indexAtacante] || pokeAtacante;
      const pokDefFB   = est.pokemonData?.[defensor]?.[indexDefensor]  || pokeDefensor;
      ...
      const { puedeActuar, nuevosEstadosPost } = verificarEstadoPreAccion(
        nuevosEstados, atacante, indexAtacante
      );
      ...
      if (!puedeActuar) {
        return {
          ...est,
          hp: nuevosHP, pp: nuevosPP, estados: nuevosEstados, estadisticas: nuevosStats,
          turno: defensor, fase: "elegir",
      ...
      // ── Verificar precisión ──
      const precMod = obtenerModificadorPrecision(nuevosStats, atacante, indexAtacante, defensor, indexDefensor);
      ...
      if (movFB.clase === "status") {
        ...
        const r = aplicarMovimientoStatus(movFB, pokAtacFB, pokDefFB, nuevosEstados, defensor, indexDefensor);
        logMsg        = r.mensaje;
        ...
        if (movFB.efectoEstado && danoRealizado > 0 && Math.random() < 0.3) {
          const sr = aplicarEstadoPorMovimiento(movFB.efectoEstado, pokDefFB, nuevosEstados, defensor, indexDefensor);
          if (sr.mensaje) { logMsg += " " + sr.mensaje; nuevosEstados = sr.nuevosEstados; }
        }
      }

      // ── Daño post-acción (veneno, quemadura, etc.) ──
      const postR = aplicarDanoPostAccion(
        nuevosHP, nuevosEstados, atacante, defensor,
        indexAtacante, indexDefensor, pokAtacFB, pokDefFB
      );
    ...
    if (resultado.committed) {
      ...
      const danoFinal      = calcularDanoDesdeEstados(estadoFinal, estadoAnterior, defensor, indexDefensor);
  ...
  switch (e.nombre) {
    ...
    case "DORMIDO":
      if (e.turnosRestantes <= 0) {
        nuevo[jugador][index] = { nombre: null, turnosRestantes: 0, acumulador: 0 };
...
function obtenerModificadorPrecision(estadisticas, atacante, indexAtac, defensor, indexDef) {
...
function calcularDano(mov, pokAtac, pokDef, estadisticas, estados,
    atacanteN, defensorN, idxAtac, idxDef, clima, hp, esCritico) {
  ...
  if (estados?.[atacanteN]?.[idxAtac]?.nombre === "QUEMADO" && mov.clase === "physical") atk *= 0.5;
  ...
  let eff = 1;
  for (const tipo of (pokDef.tipos || ["normal"])) eff *= TIPO_CHART[mov.tipo]?.[tipo] ?? 1;
  ...
  let dano = Math.floor(
    ((((2 * nivel / 5) + 2) * Math.max(1, mov.poder) * atk / Math.max(1, def)) / 50 + 2)
    * stab * eff * weatherMod * critMod * random
  );
  ...
  if (eff === 0)           { logMsg += " ¡No le afecta!"; dano = 0; }
...
function aplicarMovimientoStatus(mov, pokAtac, pokDef, estados, defensorN, idxDef) {
  const nuevo = structuredClone(estados);
  ...
  if (mov.efectoEstado && !e.nombre) {
    let turnos = 0, acum = 0;
    if (mov.efectoEstado === "DORMIDO")      turnos = Math.floor(Math.random() * 3) + 1;
    if (mov.efectoEstado === "VENENO_GRAVE") acum   = 1;
    nuevo[defensorN][idxDef] = { nombre: mov.efectoEstado, turnosRestantes: turnos, acumulador: acum };
    return { mensaje: `${pokDef.nombre} quedó ${mov.efectoEstado}!`, nuevosEstados: nuevo };
  }
  return { mensaje: `${pokAtac.nombre} usó ${mov.nombre}, pero no tuvo efecto.`, nuevosEstados: nuevo };
}
...
function aplicarEstadoPorMovimiento(estado, pokObj, estados, jugador, index) {
  ...
  if (!nuevo[jugador]?.[index]?.nombre) {
    ...
    nuevo[jugador][index] = { nombre: estado, turnosRestantes: turnos, acumulador: acum };
    return { mensaje: `${pokObj.nombre} quedó ${estado}!`, nuevosEstados: nuevo };
  }
  return { mensaje: "", nuevosEstados: nuevo };
}

function aplicarDanoPostAccion(hp, estados, atacante, defensor, idxAtac, idxDef, pokAtac, pokDef) {
  ...
  return { mensaje: msgs.join(" "), nuevosHPPost: nuevosHP, nuevosEstadosPost2: nuevosEstados };
}
...
function mostrarBolsa() {
  ...
  miEquipo.forEach((poke, i) => {
    ...
    const disabled = !efecto || (objeto.id !== "revive" && objeto.id !== "max-revive" && hpActual <= 0);
    hayObjetos     = true;
    ...
    await runTransaction(combateRef, (est) => {
      ...
      switch (efecto.tipo) {
        case "hp":
          ...
          nuevosHP[miNombre][indexPoke] = Math.min(hpMax, hpAct + efecto.cantidad);
        ...
        case "estado_especifico":
          if (est.estados[miNombre][indexPoke].nombre !== efecto.estado) return est;
          nuevosEstados[miNombre][indexPoke] = { nombre: null, turnosRestantes: 0, acumulador: 0 };
        ...
        case "pp":
          ...
          break;
        case "pp_full":
          nuevosPP[miNombre][indexPoke] = miEquipo[indexPoke].movimientos.map(m => m.ppMax);
...
class AnimacionBatalla {
  ...
  _init() {
    if (!document.getElementById("animacion-overlay")) {
      ...
      el.className= "oculto";
      el.innerHTML= '<div class="efecto-visual"></div><div class="texto-animacion"></div>';
  ...
  async reproducirAtaque(mov, esEspecial = false) {
    ...
    const cfg    = ANIMACIONES_MOVIMIENTO[clase]?.[tipo] || ANIMACIONES_MOVIMIENTO.physical.normal;
